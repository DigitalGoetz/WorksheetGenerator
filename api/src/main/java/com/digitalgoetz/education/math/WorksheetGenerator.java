...
public class WorksheetGenerator {
	...
	private void addProblem(OperationType op, int digits, int leftMargin, int columnDepth) throws IOException {
		...
		if (firstNum.trim().equals("0") && secondNum.trim().equals("0")) {
			firstNum = this.numbers.getNumber(digits, true);
			secondNum = this.numbers.getNumber(digits, true);
		}

		if (firstNum.trim().equals("0")) {
			if (this.zeroCount > 0) {
				firstNum = this.numbers.getNumber(digits, true);
			} else {
				this.zeroCount++;
	...
	private void buildColumn(OperationType type, int digits, int leftSpace, int height, int spacing)
			throws IOException {
		while (height > 75) {
	...
	public static File createWorksheet(OperationType type, int numberOfDigits, boolean allowNegatives) throws IOException {
		WorksheetGenerator gen = new WorksheetGenerator(!allowNegatives, false);
